...
class Product {
    ...
    public function saveProduct(bool $update = false) {

        $currentSeller = $this->seller_id;
        $productName = str_replace(' ', '', $this->product_name);

        //create a directory for user if it doesn't exist
        if (!is_dir(__DIR__ . "/../../public/img/userImg/$currentSeller")) {
            mkdir(__DIR__ . "/../../public/img/userImg/$currentSeller");
        }

        //create a dir for product if doesn't exist
        if (!is_dir(__DIR__ . "/../../public/img/userImg/$currentSeller" . "/$productName")) {
